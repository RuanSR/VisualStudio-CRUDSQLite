...
namespace CRUDSQLite.Classes.DB
{
    public static class Query
    {
        ...
        public static string SelectUser(string nome)
        {
            return $@"SELECT {nameof(User.Id)}, {nameof(User.Nome)}, {nameof(User.Nascimento)}, {nameof(User.RU)}, {nameof(User.Genero)}, {nameof(User.Obs)} FROM {nameof(User)} WHERE ({nameof(User.Nome)} Like '%{nome}%' )";
        }
    }
}
